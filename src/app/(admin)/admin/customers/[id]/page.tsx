...
export default function CustomerDetailPage() {
  ...
  async function handleRedeem() {
    if (!customer) return;
    if (!confirm(`¿Canjear recompensa para ${customer.name}?`)) return;
  ...
  return (
    <div className="p-4 max-w-lg mx-auto pb-8">
      <button onClick={() => router.back()} className="text-coffee-medium text-sm mt-4 mb-4">
        ← Volver
      </button>
      ...
      <div className="loyalty-card rounded-2xl p-5 text-white mb-4 relative z-10">
        ...
        <div className="flex justify-between mt-4">
          <div>
            <p className="text-coffee-light text-xs">Saldo</p>
            <p className="text-xl font-bold">{customer.balanceMXN}</p>
          </div>
          <div className="text-right">
            <p className="text-coffee-light text-xs">Visitas</p>
            <p className="text-xl font-bold">{customer.visitsThisCycle}/{customer.visitsRequired}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        <div className="card-surface text-center">
          ...
          <p className="text-xs text-coffee-medium">Total visitas</p>
        </div>
        <div className="card-surface text-center">
          <p className="text-2xl font-bold text-coffee-dark">{customer.pendingRewards}</p>
          <p className="text-xs text-coffee-medium">Recompensas</p>
        </div>
        <div className="card-surface text-center">
          <p className="text-2xl font-bold text-coffee-dark">{customer.balanceMXN}</p>
          <p className="text-xs text-coffee-medium">Saldo</p>
        </div>
      </div>

      {message && (
        <div className={`rounded-xl p-3 mb-4 text-sm font-medium text-center ${
          messageIsSuccess ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'
        }`}>
          {message}
        </div>
      )}
        ...
        <form onSubmit={handleTopUp} className="space-y-3">
            ...
            {COMMON_TOPUP_AMOUNTS.map(({ label, centavos }) => (
              <button
                ...
                className={`py-2 rounded-xl text-sm font-medium transition-colors ${
                  topupAmount === String(centavos / 100)
          ...
          <div className="flex gap-2">
            <input
              ...
              max="10000"
            />
            <button type="submit" disabled={topupLoading || !topupAmount} className="btn-primary">
      ...
      {customer.pendingRewards > 0 && (
        <div className="card-surface border-2 border-coffee-brand/30 mb-4">
          ...
          >
            {redeemLoading ? 'Canjeando...' : 'Canjear recompensa'}
          </button>
        </div>
      )}
