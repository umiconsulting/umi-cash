...
export default function TopUpPage() {
  ...
  function selectAmount(centavos: number) {
    setAmount(String(centavos / 100));
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      try {
        amountCentavos = centavosFromPesos(amount);
      } catch {
      ...
      if (amountCentavos < 100) {
        setResult({ success: false, message: 'El monto mínimo es $1.00 MXN' });
        return;
      }
      ...
      if (res.ok) {
        setResult({
          success: true,
          message: `Recarga exitosa: ${data.amountMXN}`,
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="card-surface">
          ...
          <p className="text-xs text-coffee-light mt-1">
            Busca el número en la tarjeta digital del cliente
          </p>
        </div>
        ...
        {result && (
          <div
            className={`card-surface border-2 text-center ${
          ...
          >
            ...
            {result.success && result.newBalanceMXN && (
              <p className="text-coffee-dark mt-2">
                Nuevo saldo:{' '}
                <span className="font-bold text-lg">{result.newBalanceMXN}</span>
              </p>
            )}
            {result.customer && (
              <p className="text-sm text-gray-600 mt-1">Cliente: {result.customer}</p>
            )}
          </div>
        )}
        ...
        <button
          type="submit"
          disabled={loading || !cardId || !amount}
