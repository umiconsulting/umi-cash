...
export default function RewardsPage() {
  ...
  async function handleSave(e: React.FormEvent) {
    e.preventDefault();
    if (role !== 'ADMIN') {
      setMessage('Solo los administradores pueden cambiar las recompensas.');
      return;
    }

    const confirmed = window.confirm(
      `¿Cambiar la recompensa a "${form.rewardName}"?\n\nEl progreso de los clientes se conserva.`
    );
    if (!confirmed) return;
    ...
    const data = await res.json();
    if (res.ok) {
      setMessage('Recompensa actualizada');
  ...
  // Preset rewards for quick selection
  const PRESETS = [
    'Cookie de temporada',
    'Americano helado',
    'Latte helado',
  ...
  return (
    <div className="p-4 max-w-lg mx-auto">
      <h1 className="font-display text-2xl font-bold text-coffee-dark mt-4 mb-2">
        Recompensas
      </h1>
      ...
      {active && (
        <div className="loyalty-card rounded-2xl p-5 text-white mb-6 relative z-10">
          <p className="text-coffee-light text-xs uppercase tracking-widest">
            Recompensa activa
          </p>
          <h2 className="font-display text-xl font-bold mt-1">{active.rewardName}</h2>
          <p className="text-coffee-light mt-1 text-sm">
            Cada {active.visitsRequired} visitas
          </p>
          {active.rewardDescription && (
            <p className="text-coffee-pale/70 text-xs mt-2">{active.rewardDescription}</p>
          )}
        </div>
      )}
      ...
      {role === 'ADMIN' ? (
        <form onSubmit={handleSave} className="space-y-4">
                ...
                <button
                  ...
                  className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${
                    form.rewardName === p
                      ...
                      : 'bg-coffee-pale text-coffee-medium hover:bg-coffee-light/30'
                  }`}
                >
                  {p}
                </button>
          ...
          {message && (
            <div className={`text-center text-sm font-medium ${messageIsSuccess ? 'text-green-700' : 'text-red-700'}`}>
              {message}
            </div>
          )}
          ...
          <button
            type="submit"
            disabled={saving || !form.rewardName}
          ...
          <div className="space-y-2">
            {history.map((h) => (
              <div key={h.id} className="card-surface flex justify-between items-center text-sm">
                <div>
                  <p className="font-medium text-coffee-dark">{h.rewardName}</p>
                  <p className="text-coffee-light">Cada {h.visitsRequired} visitas</p>
                </div>
                <div className="text-right text-coffee-light text-xs">
